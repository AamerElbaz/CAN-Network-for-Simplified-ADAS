...
static boolean ReceiveApplicationOverHeads(uint16_t Buffer_RX)
{
...
static void Decode_Message(RuntimeParametersType* g_RuntimeParametersPtr)
{
    ...
    if (TRUE == g_RuntimeParametersPtr->StateMachine->StatusChanged)
    {
        ...
        if( TRUE == ReceiveApplicationOverHeads ( Buffer_RX ) )
        {
            UltrasonicSensorReading = (Buffer_RX << 6);
            UltrasonicSensorReading = (UltrasonicSensorReading >> 13);
        }
        else
        {
            /* MISRA RULE */
    ...
    else if ( TRUE == ReceiveApplicationOverHeads ( Buffer_RX ) )
    {
        UltrasonicSensorReading = (Buffer_RX << 6);
        UltrasonicSensorReading = (UltrasonicSensorReading >> 13);
    }
